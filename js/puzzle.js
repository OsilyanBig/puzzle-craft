class PuzzleGame {
    constructor(canvasEl, imageUrl, pieceCount, puzzleId) {
        ...
        // İpucu sistemi
        this.hintsTotal = pieceCount <= 50 ? 5 : pieceCount <= 100 ? 4 : 3;
        this.hintsUsed = 0;
        this.hintsRemaining = this.hintsTotal;
    ...
    load(savedState) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            self.img.onload = function () {
                self._setup();
                if (savedState && savedState.pieces && savedState.edgesH) {
    ...
    // ─── İPUCU ───
    useHint() {
        if (this.hintsRemaining <= 0 || this.completed) return false;

        // Yerleşmemiş rastgele bir parça bul
        var unplaced = [];
        for (var i = 0; i < this.pieces.length; i++) {
            if (!this.pieces[i].placed) unplaced.push(this.pieces[i]);
        }
        if (unplaced.length === 0) return false;

        var target = unplaced[Math.floor(Math.random() * unplaced.length)];
        var gid = target.group;

        // Tüm grubu doğru yerine yerleştir
        for (var j = 0; j < this.pieces.length; j++) {
            if (this.pieces[j].group === gid) {
                this.pieces[j].x = this.pieces[j].cx;
                this.pieces[j].y = this.pieces[j].cy;
                this.pieces[j].placed = true;
            }
        }

        // Çevredeki parçalarla birleş
        this._chainMerge(gid);
    ...
    _updateHintUI() {
        ...
        if (el) el.textContent = this.hintsRemaining;
        if (btn) {
    ...
    _playSound(type) {
        try {
            var el = document.getElementById('snd-' + type);
            if (el) {
                el.currentTime = 0;
                el.volume = 0.3;
    ...
    _draw() {
        ...
        for (var c = 1; c < this.cols; c++) {
            ctx.beginPath(); ctx.moveTo(c * this.pw, 0); ctx.lineTo(c * this.pw, this.imgH); ctx.stroke();
        }
        for (var r = 1; r < this.rows; r++) {
            ctx.beginPath(); ctx.moveTo(0, r * this.ph); ctx.lineTo(this.imgW, r * this.ph); ctx.stroke();
        }
        ctx.restore();
    ...
    _drawPiece(p) {
        ...
        if (destW > 0 && destH > 0) {
            ctx.drawImage(this.img, sx2, sy2, sRight - sx2, sBottom - sy2, destX, destY, destW, destH);
        }
        ...
        this._piecePath(ctx, p);
        ctx.closePath();
        ctx.strokeStyle = p.placed ? 'rgba(0,184,148,0.5)' : (p.edge ? 'rgba(162,155,254,0.6)' : 'rgba(255,255,255,0.45)');
    ...
    _piecePath(ctx, p) {
        var pw = this.pw, ph = this.ph, tab = this.tab;
        var neck = 0.5, tabW = 0.3, d;

        ctx.moveTo(0, 0);

        if (p.top === 0) { ctx.lineTo(pw, 0); }
        else {
            d = p.top;
            ctx.lineTo(pw * (0.5 - tabW / 2), 0);
            ctx.bezierCurveTo(pw * (0.5 - tabW / 2), d * -tab * neck, pw * (0.5 - tabW / 2 - 0.04), d * -tab, pw * 0.5, d * -tab);
            ctx.bezierCurveTo(pw * (0.5 + tabW / 2 + 0.04), d * -tab, pw * (0.5 + tabW / 2), d * -tab * neck, pw * (0.5 + tabW / 2), 0);
            ctx.lineTo(pw, 0);
        }

        if (p.right === 0) { ctx.lineTo(pw, ph); }
        else {
            d = p.right;
            ctx.lineTo(pw, ph * (0.5 - tabW / 2));
            ctx.bezierCurveTo(pw + d * tab * neck, ph * (0.5 - tabW / 2), pw + d * tab, ph * (0.5 - tabW / 2 - 0.04), pw + d * tab, ph * 0.5);
            ctx.bezierCurveTo(pw + d * tab, ph * (0.5 + tabW / 2 + 0.04), pw + d * tab * neck, ph * (0.5 + tabW / 2), pw, ph * (0.5 + tabW / 2));
            ctx.lineTo(pw, ph);
        }

        if (p.bottom === 0) { ctx.lineTo(0, ph); }
        else {
            d = p.bottom;
            ctx.lineTo(pw * (0.5 + tabW / 2), ph);
            ctx.bezierCurveTo(pw * (0.5 + tabW / 2), ph + d * tab * neck, pw * (0.5 + tabW / 2 + 0.04), ph + d * tab, pw * 0.5, ph + d * tab);
            ctx.bezierCurveTo(pw * (0.5 - tabW / 2 - 0.04), ph + d * tab, pw * (0.5 - tabW / 2), ph + d * tab * neck, pw * (0.5 - tabW / 2), ph);
            ctx.lineTo(0, ph);
        }

        if (p.left === 0) { ctx.lineTo(0, 0); }
        else {
            d = p.left;
            ctx.lineTo(0, ph * (0.5 + tabW / 2));
            ctx.bezierCurveTo(d * -tab * neck, ph * (0.5 + tabW / 2), d * -tab, ph * (0.5 + tabW / 2 + 0.04), d * -tab, ph * 0.5);
            ctx.bezierCurveTo(d * -tab, ph * (0.5 - tabW / 2 - 0.04), d * -tab * neck, ph * (0.5 - tabW / 2), 0, ph * (0.5 - tabW / 2));
            ctx.lineTo(0, 0);
        }
    }
    ...
    _bindInput() {
        ...
        c.addEventListener('mousedown', function (e) { e.preventDefault(); self._down(e.offsetX, e.offsetY, e.button); });
        c.addEventListener('mousemove', function (e) { self._move(e.offsetX, e.offsetY); });
        c.addEventListener('mouseup', function () { self._up(); });
        c.addEventListener('mouseleave', function () { self._up(); });
        c.addEventListener('wheel', function (e) { e.preventDefault(); self._wheel(e); }, { passive: false });
        ...
        c.addEventListener('touchmove', function (e) {
            ...
            if (e.touches.length === 1 && self.pointerDown) {
                ...
                self.panX += mid.x - lastTouchMid.x;
                self.panY += mid.y - lastTouchMid.y;
                lastTouchDist = dist;
                lastTouchMid = mid;
            }
        }, { passive: false });
        ...
        c.addEventListener('touchend', function (e) {
            e.preventDefault();
            if (e.touches.length === 0) { self._up(); lastTouchDist = 0; }
    ...
    _hitTest(wx, wy) {
        ...
        for (var i = this.pieces.length - 1; i >= 0; i--) {
            ...
            if (wx >= p.x - margin && wx <= p.x + this.pw + margin && wy >= p.y - margin && wy <= p.y + this.ph + margin) return p;
        }
        return null;
    }
    ...
    _down(sx, sy, btn) {
        ...
        if (hit) {
            ...
            var gid = hit.group, inGroup = [], rest = [];
            for (var i = 0; i < this.pieces.length; i++) {
                if (this.pieces[i].group === gid) inGroup.push(this.pieces[i]);
                else rest.push(this.pieces[i]);
            }
    ...
    _move(sx, sy) {
        ...
        if (this.isPanning) { this.panX = sx - this.panStartX; this.panY = sy - this.panStartY; return; }
        if (this.dragPiece) {
            var w = this._screenToWorld(sx, sy);
            var dx = w.x - this.dragOffX - this.dragPiece.x;
            var dy = w.y - this.dragOffY - this.dragPiece.y;
            var gid = this.dragPiece.group;
            for (var i = 0; i < this.pieces.length; i++) {
                if (this.pieces[i].group === gid) { this.pieces[i].x += dx; this.pieces[i].y += dy; }
            }
        }
    }
    ...
    _trySnap(piece) {
        ...
        for (i = 0; i < groupPieces.length; i++) {
            ...
            if (Math.abs(gp.cx - gp.x) < this.snapDist && Math.abs(gp.cy - gp.y) < this.snapDist) {
        ...
        for (i = 0; i < groupPieces.length; i++) {
            ...
            for (j = 0; j < neighbors.length; j++) {
                ...
                if (Math.abs((nb.x - gp2.x) - expDx) < this.snapDist && Math.abs((nb.y - gp2.y) - expDy) < this.snapDist) {
    ...
    _findPiece(id) {
        for (var i = 0; i < this.pieces.length; i++) { if (this.pieces[i].id === id) return this.pieces[i]; }
        return null;
    }
    ...
    _merge(gidA, gidB, pieceA, neighborB) {
        ...
        var i;
        for (i = 0; i < this.pieces.length; i++) {
            if (this.pieces[i].group === gidA) {
                this.pieces[i].x += shiftX; this.pieces[i].y += shiftY;
                this.pieces[i].group = gidB;
                if (isPlaced) { this.pieces[i].x = this.pieces[i].cx; this.pieces[i].y = this.pieces[i].cy; this.pieces[i].placed = true; }
            }
        }
        var anyPlaced = false;
        for (i = 0; i < this.pieces.length; i++) { if (this.pieces[i].group === gidB && this.pieces[i].placed) { anyPlaced = true; break; } }
        if (anyPlaced) {
            for (i = 0; i < this.pieces.length; i++) {
                if (this.pieces[i].group === gidB) { this.pieces[i].x = this.pieces[i].cx; this.pieces[i].y = this.pieces[i].cy; this.pieces[i].placed = true; }
            }
        }
    ...
    _chainMerge(gid) {
        ...
        while (found) {
            ...
            var groupPieces = [];
            var i, j, k;
            for (i = 0; i < this.pieces.length; i++) { if (this.pieces[i].group === gid) groupPieces.push(this.pieces[i]); }
            for (i = 0; i < groupPieces.length; i++) {
                var gp = groupPieces[i];
                ...
                for (j = 0; j < nids.length; j++) {
                    ...
                    if (!nb || nb.group === gid) continue;
                    if (Math.abs(nb.x - gp.x - (nb.c - gp.c) * this.pw) < 2 && Math.abs(nb.y - gp.y - (nb.r - gp.r) * this.ph) < 2) {
                        var oldGid = nb.group;
                        var isPlaced = gp.placed || nb.placed;
                        for (k = 0; k < this.pieces.length; k++) {
                            if (this.pieces[k].group === oldGid) {
                                this.pieces[k].group = gid;
                                if (isPlaced) { this.pieces[k].x = this.pieces[k].cx; this.pieces[k].y = this.pieces[k].cy; this.pieces[k].placed = true; }
                            }
                        }
                        found = true;
                    }
    ...
    _checkComplete() {
        var placedCount = 0;
        for (var i = 0; i < this.pieces.length; i++) { if (this.pieces[i].placed) placedCount++; }
        if (placedCount === this.pieces.length && !this.completed) {
            ...
            if (this.onComplete) {
                setTimeout(function () {
                    self.onComplete(self._fmtTime(self.elapsed), self.pieces.length, self.elapsed, self.hintsUsed);
                }, 600);
            }
        }
    }
    ...
    // ─── UI ───
    _updateUI() {
        var placed = 0, total = this.pieces.length || this.totalPieces || 0;
        for (var i = 0; i < this.pieces.length; i++) { if (this.pieces[i].placed) placed++; }
    ...
    _autoSave() {
        ...
        var placed = 0, i;
        for (i = 0; i < this.pieces.length; i++) { if (this.pieces[i].placed) placed++; }
        var piecesData = [];
        for (i = 0; i < this.pieces.length; i++) {
            var p = this.pieces[i];
            piecesData.push({ id: p.id, r: p.r, c: p.c, x: p.x, y: p.y, cx: p.cx, cy: p.cy, group: p.group, placed: p.placed });
        }
        Storage.savePuzzle(this.puzzleId, {
            pieces: piecesData, edgesH: this.edgesH, edgesV: this.edgesV,
            cols: this.cols, rows: this.rows, elapsed: this.elapsed,
            panX: this.panX, panY: this.panY, scale: this.scale,
            totalPieces: this.pieces.length, placedCount: placed,
            progress: Math.round((placed / this.pieces.length) * 100),
            hintsUsed: this.hintsUsed, hintsRemaining: this.hintsRemaining
        });
    }

    _restore(saved) {
        this.cols = saved.cols; this.rows = saved.rows;
        this.edgesH = saved.edgesH; this.edgesV = saved.edgesV;
        ...
        if (saved.hintsUsed !== undefined) { this.hintsUsed = saved.hintsUsed; this.hintsRemaining = saved.hintsRemaining; }

        this.totalPieces = this.cols * this.rows;
        this.pw = this.imgW / this.cols; this.ph = this.imgH / this.rows;
        this.tab = Math.min(this.pw, this.ph) * 0.2;
        this.snapDist = Math.min(this.pw, this.ph) * (window.innerWidth < 768 ? 0.45 : 0.35);
        this._generate();
        for (var i = 0; i < saved.pieces.length; i++) {
            var sp = saved.pieces[i], p = this._findPiece(sp.id);
            if (p) { p.x = sp.x; p.y = sp.y; p.group = sp.group; p.placed = sp.placed; }
        }
    }
    ...
    destroy() { this.running = false; if (this.timerInterval) clearInterval(this.timerInterval); }
}
